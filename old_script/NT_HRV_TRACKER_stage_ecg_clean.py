...
def compute_ecg(raw_ecg, srate, parameter_preset='human_ecg', parameters=None):
    ...
    print(clean_ecg.mean())
    print(params_detection['thresh'])
    print(raw_ecg_peak)
    plt.plot(clean_ecg)
    plt.hlines(params_detection['thresh'], 0, len(clean_ecg),colors='r')
    plt.show()
